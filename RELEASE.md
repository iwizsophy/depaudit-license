...
- [CHANGELOG.md](CHANGELOG.md) に `1.3.0` の section があり、review-only catalog entries、NuGet embedded file license fallback、artifact safety limits が記載されている
- [README.md](README.md) と [README.ja.md](README.ja.md) に review-only catalog definitions の扱い、NuGet embedded file license の解決方針、外部ネットワークアクセス先、HTTP cache、endpoint override、API key / token 設定、artifact safety limits、runtime config (`-config`) の優先順位、`provenance.externalSources` の意味が記載されている
- [configs/README.md](configs/README.md) に review-only catalog entries と `requires_manual_review` の使い分け、および runtime config schema / sample の位置づけが記載されている
